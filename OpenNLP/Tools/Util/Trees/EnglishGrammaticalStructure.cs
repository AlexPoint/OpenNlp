  ...
  private static void collapseFlatMWP(List<TypedDependency> list) {
    ...
    foreach (String[] mwp in MULTIWORD_PREPS) {
      ...
      foreach (TypedDependency td in list) {
        if (Math.Abs(td.gov().index() - td.dep().index()) == 1 && td.gov().value().Equals(mwp[1], StringComparison.InvariantCultureIgnoreCase)
    ...
    foreach (String[] mwp in MULTIWORD_PREPS) {
      ...
      foreach (TypedDependency td in list) {
        if (td.dep().value().Equals(mwp[0], StringComparison.InvariantCultureIgnoreCase) 
            && (td.reln() == EnglishGrammaticalRelations.PHRASAL_VERB_PARTICLE || td.reln() == EnglishGrammaticalRelations.ADVERBIAL_MODIFIER 
            || td.reln() == GrammaticalRelation.DEPENDENT || td.reln() == EnglishGrammaticalRelations.MULTI_WORD_EXPRESSION)) {
      ...
      foreach (TypedDependency td1 in list) {
        if (td1.reln() == EnglishGrammaticalRelations.PREPOSITIONAL_MODIFIER 
            ...
            && Math.Abs(td1.dep().index() - mwp0.index()) == 1 && td1.gov().Equals(governor)) {// we
          // found
          // prep(gov,
          // mwp1)
          mwp1 = td1.dep();
...
*/
  private static void collapse3WP(List<TypedDependency> list) {
    ...
    foreach (String[] mwp in THREEWORD_PREPS) {
      ...
      foreach (TypedDependency td in list) {
        if (td.gov().Equals(mwp1) && td.dep().value().Equals(mwp[2], StringComparison.InvariantCultureIgnoreCase)
            && Math.Abs(td.gov().index() - td.dep().index()) == 1) {
          mwp2 = td.dep();
          dep2 = td;
        }
      ...
      if (dep1 != null && dep2 != null) {
        ...
        TypedDependency prep = null;
        foreach (TypedDependency td1 in list) {
          if (td1.reln() == EnglishGrammaticalRelations.PREPOSITIONAL_MODIFIER && td1.dep().Equals(mwp0)) {// we
            // found
            // prep(gov,
            // mwp0)
            prep = td1;
    ...
    foreach (String[] mwp in THREEWORD_PREPS) {
      ...
      // indexes = 2)
      foreach (TypedDependency td in list) {
        if (td.gov().Equals(mwp0) && td.dep().value().Equals(mwp[2], StringComparison.InvariantCultureIgnoreCase)
      ...
      if (dep1 != null && dep2 != null) {
        ...
        TypedDependency prep = null;
        foreach (TypedDependency td1 in list) {
          if (td1.dep().Equals(mwp0) && td1.reln() == EnglishGrammaticalRelations.PREPOSITIONAL_MODIFIER) {// we
            // found
            // prep(gov,
            // mwp0)
            prep = td1;
  ...
  private static void collapsePrepAndPoss(List<TypedDependency> list) {
    ...
    Dictionary<IndexedWord, SortedSet<TypedDependency>> map = new Dictionary<IndexedWord, SortedSet<TypedDependency>>();
    List<IndexedWord> vmod = new List<IndexedWord>();
    ...
    foreach (TypedDependency typedDep in list) {
      ...
      if (typedDep.reln() == EnglishGrammaticalRelations.VERBAL_MODIFIER) {
        ...
        foreach (TypedDependency auxDep in list) {
          ...
          if (!auxDep.gov().Equals(typedDep.dep()) || !auxDep.dep().value().Equals("to", StringComparison.InvariantCultureIgnoreCase)) {
            continue;
          }
          foundAux = true;
          break;
    ...
    foreach (TypedDependency td1 in list) {
      ...
      List<Tuple<TypedDependency, TypedDependency, bool>> conjs = new List<Tuple<TypedDependency, TypedDependency, bool>>();
      ...
      foreach (TypedDependency td2 in possibles) {
        ...
        if (td2.reln() == EnglishGrammaticalRelations.COORDINATION && td2.dep().index() < conjIndex) {
          ...
          if ((td2.reln() == GrammaticalRelation.DEPENDENT || td2.reln() == EnglishGrammaticalRelations.PREPOSITIONAL_OBJECT || td2.reln() == EnglishGrammaticalRelations.PREPOSITIONAL_COMPLEMENT) && (td1DepPOS.Equals("IN") || td1DepPOS.Equals("TO") || td1DepPOS.Equals("VBG")) && prepDep == null && (!(td2DepPOS.Equals("RB") || td2DepPOS.Equals("IN") || td2DepPOS.Equals("TO")))) {
            ...
            // td2.dep().index() < index)
            prepDep = new Tuple<TypedDependency, Boolean>(td2, td2.reln() != EnglishGrammaticalRelations.PREPOSITIONAL_COMPLEMENT);
      ...
      if (samePrepositionInEachConjunct) { // conjDep != null && prepOtherDep !=
        ...
        foreach (Tuple<TypedDependency, TypedDependency, Boolean> trip in conjs) {
          ...
          if (prepOtherDep == null) {
            ...
            TypedDependency tdNew2 = new TypedDependency(conjValue(ccDep.dep().value()), prepDep.Item1.dep(), prepOtherDep.dep());
        ...
        foreach (TypedDependency td2 in possibles) {
          ...
          // }
          if (td2.reln() != GrammaticalRelation.KILL && td2.gov().Equals(td1.dep())) { // && td2.reln()
            // != COORDINATION
            // && td2.reln()
            // != CONJUNCT
            /*if (DEBUG) {
      ...
      // it does, since they're not automatically deleted
      foreach (TypedDependency td2 in possibles) {
        if (td2.reln() != GrammaticalRelation.KILL) { // && td2.reln() != COORDINATION &&
          // td2.reln() != CONJUNCT) {
          /*if (DEBUG) {
    ...
    foreach (TypedDependency td1 in list) {
      ...
      if (possibles != null && (td1.reln() == EnglishGrammaticalRelations.PREPOSITIONAL_MODIFIER || td1.reln() == EnglishGrammaticalRelations.POSSESSION_MODIFIER || td1.reln() == EnglishGrammaticalRelations.CONJUNCT)) {
        ...
        foreach (TypedDependency td2 in possibles) {
          if (td2.reln() != EnglishGrammaticalRelations.COORDINATION && td2.reln() != EnglishGrammaticalRelations.CONJUNCT) {
            ...
            } else if ((td2.reln() == EnglishGrammaticalRelations.PREPOSITIONAL_OBJECT || td2.reln() == EnglishGrammaticalRelations.PREPOSITIONAL_COMPLEMENT) && (td1DepPOS.Equals("IN") || td1DepPOS.Equals("TO") || td1DepPOS.Equals("VBG")) && (!(td2DepPOS.Equals("RB") || td2DepPOS.Equals("IN") || td2DepPOS.Equals("TO"))) && !isConjWithNoPrep(td2.gov(), possibles)) {
   ...
   */
  private static GrammaticalRelation determinePrepRelation(Dictionary<IndexedWord, SortedSet<TypedDependency>> map, List<IndexedWord> vmod, TypedDependency pc, TypedDependency topPrep, bool pobj) {
...
*/
  private static void collapseConj(List<TypedDependency> list) {
    List<IndexedWord> govs = new List<IndexedWord>();
    // find typed deps of form cc(gov, dep)
    foreach (TypedDependency td in list) {
      if (td.reln() == EnglishGrammaticalRelations.COORDINATION) { // i.e. "cc"
        ...
        foreach (TypedDependency td1 in list) {
          if (td1.gov().Equals(gov)) {
            if (td1.reln() == EnglishGrammaticalRelations.CONJUNCT) { // i.e., "conj"
...
*/
  private static void addRef(List<TypedDependency> list) {
    List<TypedDependency> newDeps = new List<TypedDependency>();
  ...
  private static void collapseReferent(List<TypedDependency> list) {
    ...
    foreach (TypedDependency re in refs) {
      ...
      foreach (TypedDependency td in list) {
        ...
        if (td.dep().Equals(dep) && td.reln() != EnglishGrammaticalRelations.REFERENT && !td.gov().Equals(ant)) {
          /*if (DEBUG)
    ...
    Dictionary<IndexedWord, Set<TypedDependency>> map = new Dictionary<IndexedWord, Set<TypedDependency>>();
    // Construct a map of tree nodes being governor of a subject grammatical
    // relation to that relation
    Dictionary<IndexedWord, TypedDependency> subjectMap = new Dictionary<IndexedWord, TypedDependency>();
    // Construct a set of TreeGraphNodes with a passive auxiliary on them
    Set<IndexedWord> withPassiveAuxiliary = new HashSet<IndexedWord>();
    ...
    List<IndexedWord> rcmodHeads = new List<IndexedWord>();
    List<IndexedWord> prepcDep = new List<IndexedWord>();
    ...
    foreach (TypedDependency typedDep in list) {
      ...
      // look for subjects
      if (typedDep.reln().getParent() == EnglishGrammaticalRelations.NOMINAL_SUBJECT || typedDep.reln().getParent() == EnglishGrammaticalRelations.SUBJECT || typedDep.reln().getParent() == EnglishGrammaticalRelations.CLAUSAL_SUBJECT) {
    ...
    List<TypedDependency> newTypedDeps = new List<TypedDependency>(list);
    ...
    foreach (TypedDependency td in list) {
      if (EnglishGrammaticalRelations.getConjs().Contains(td.reln())) {
            ...
            if (newRel != GrammaticalRelation.ROOT) {
              if (rcmodHeads.Contains(gov) && rcmodHeads.Contains(dep)) {
                // to prevent wrong propagation in the case of long dependencies in relative clauses
                  if (newRel != EnglishGrammaticalRelations.DIRECT_OBJECT && newRel != EnglishGrammaticalRelations.NOMINAL_SUBJECT)
                  {
                  /*if (DEBUG) {
        ...
        if (subjectMap.ContainsKey(gov) && (tag.StartsWith("VB") || tag.StartsWith("JJ")) && ! subjectMap.ContainsKey(dep)) {
          ...
          else if (relation == EnglishGrammaticalRelations.CLAUSAL_SUBJECT)
          {
            if (withPassiveAuxiliary.Contains(dep)) {
...
*/
  private static void addExtraNSubj(List<TypedDependency> list) {
    ...
    foreach (TypedDependency xcomp in list) {
      ...
      bool hasAux = false;
      List<IndexedWord> subjects = new List<IndexedWord>();
      List<IndexedWord> objects = new List<IndexedWord>();
      ...
      if (objects.Count > 0) {
        foreach (IndexedWord obj in objects) {
          TypedDependency newDep = new TypedDependency(EnglishGrammaticalRelations.NOMINAL_SUBJECT, modifier, obj);
          newDeps.Add(newDep);
        }
      } else {
        foreach (IndexedWord subject in subjects) {
          TypedDependency newDep = new TypedDependency(EnglishGrammaticalRelations.NOMINAL_SUBJECT, modifier, subject);
          newDeps.Add(newDep);
        }
      }
    }
  ...
  private static void removeDep(List<TypedDependency> list) {
    ...
    foreach (TypedDependency td1 in list) {
      if (prepRels.Contains(td1.reln())) { // if we have a prep_ relation
        ...
        foreach (TypedDependency td2 in list) {
          if (td2.reln() == GrammaticalRelation.DEPENDENT && td2.gov().Equals(gov) && td2.dep().Equals(dep)) {
            td2.setReln(GrammaticalRelation.KILL);
          }
        }
      }
  ...
  private static void eraseMultiConj(List<TypedDependency> list) {
    ...
    foreach (TypedDependency td1 in list) {
      if (td1.reln() == EnglishGrammaticalRelations.COORDINATION) {
        ...
        foreach (TypedDependency td2 in list) {
          if (td2.gov().Equals(x) && (td2.reln() == GrammaticalRelation.DEPENDENT || td2.reln() == EnglishGrammaticalRelations.MULTI_WORD_EXPRESSION || td2.reln() == EnglishGrammaticalRelations.COORDINATION ||
                  td2.reln() == EnglishGrammaticalRelations.ADVERBIAL_MODIFIER || td2.reln() == EnglishGrammaticalRelations.NEGATION_MODIFIER || td2.reln() == EnglishGrammaticalRelations.AUX_MODIFIER))
          {
              td2.setReln(GrammaticalRelation.KILL);
          }
        }
...
*/
  ...
  // ALPHABETICAL ORDER
  private static readonly String[][] MULTIWORD_PREPS = { new string[]{ "according", "to" }, new string[]{ "across", "from" },new string[] { "ahead", "of" }, new string[]{ "along", "with" }, new string[]{ "alongside", "of" }, new string[]{ "apart", "from" }, new string[]{ "as", "for" }, new string[]{ "as", "from" }, new string[]{ "as", "of" }, new string[]{ "as", "per" }, new string[]{ "as", "to" }, new string[]{ "aside", "from" }, new string[]{ "away", "from" },new string[] { "based", "on" }, new string[]{ "because", "of" }, new string[]{ "close", "by" }, new string[]{ "close", "to" }, new string[]{ "contrary", "to" }, new string[]{ "compared", "to" }, new string[]{ "compared", "with" }, new string[]{ "due", "to" }, new string[]{ "depending", "on" }, new string[]{ "except", "for" }, new string[]{ "exclusive", "of" }, new string[]{ "far", "from" }, new string[]{ "followed", "by" }, new string[]{ "inside", "of" }, new string[]{ "instead", "of" }, new string[]{ "irrespective", "of" }, new string[]{ "next", "to" }, new string[]{ "near", "to" }, new string[]{ "off", "of" }, new string[]{ "out", "of" }, new string[]{ "outside", "of" }, new string[]{ "owing", "to" }, new string[]{ "preliminary", "to" },
      new string[]{ "preparatory", "to" }, new string[]{ "previous", "to" }, new string[]{ "prior", "to" }, new string[]{ "pursuant", "to" }, new string[]{ "regardless", "of" }, new string[]{ "subsequent", "to" }, new string[]{ "such", "as" }, new string[]{ "thanks", "to" }, new string[]{ "together", "with" } };

  // used by collapse3WP() KEPT IN ALPHABETICAL ORDER
  private static readonly String[][] THREEWORD_PREPS = { new string[] { "by", "means", "of" }, new string[] { "in", "accordance", "with" }, new string[] { "in", "addition", "to" }, new string[] { "in", "case", "of" }, new string[] { "in", "front", "of" }, new string[] { "in", "lieu", "of" }, new string[] { "in", "place", "of" }, new string[] { "in", "spite", "of" }, new string[] { "on", "account", "of" }, new string[] { "on", "behalf", "of" }, new string[] { "on", "top", "of" }, new string[] { "with", "regard", "to" }, new string[] { "with", "respect", "to" } };
...
*/
  private static void collapseMultiWordPrep(List<TypedDependency> list, List<TypedDependency> newTypedDeps, String str_mwp0, String str_mwp1, String w_mwp0, String w_mwp1) {
    ...
    TypedDependency dep = null;
    foreach (TypedDependency td in list) {
      if (td.gov().value().Equals(w_mwp0, StringComparison.InvariantCultureIgnoreCase) && td.dep().value().Equals(w_mwp1, StringComparison.InvariantCultureIgnoreCase) && Math.Abs(td.gov().index() - td.dep().index()) == 1) {
        mwp0 = td.gov();
        mwp1 = td.dep();
        dep = td;
      }
    ...
    TypedDependency prep = null;
    foreach (TypedDependency td1 in list) {
      if ((td1.reln() == EnglishGrammaticalRelations.PREPOSITIONAL_MODIFIER || td1.reln() == EnglishGrammaticalRelations.ADVERBIAL_MODIFIER || td1.reln() == EnglishGrammaticalRelations.ADJECTIVAL_MODIFIER || td1.reln() == GrammaticalRelation.DEPENDENT || td1.reln() == EnglishGrammaticalRelations.MULTI_WORD_EXPRESSION) && td1.dep().Equals(mwp0)) {
    ...
    TypedDependency newtd = null;
    foreach (TypedDependency td2 in list) {
      if ((td2.reln() == EnglishGrammaticalRelations.PREPOSITIONAL_OBJECT || td2.reln() == EnglishGrammaticalRelations.PREPOSITIONAL_COMPLEMENT) && (td2.gov().Equals(mwp1) || td2.gov().Equals(mwp0))) {
