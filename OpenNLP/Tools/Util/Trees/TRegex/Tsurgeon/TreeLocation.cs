...
namespace OpenNLP.Tools.Util.Trees.TRegex.Tsurgeon
{
  ...
  public class LocationMatcher {
    Dictionary<String,Tree> newNodeNames;
    CoindexationGenerator coindexer;
      private TreeLocation location;

    TsurgeonMatcher childMatcher;

    public LocationMatcher(Dictionary<String, Tree> newNodeNames, CoindexationGenerator coindexer, TreeLocation location) {
    ...
    Tuple<Tree,int> evaluate(Tree tree, TregexMatcher tregex) {
      ...
      if (/*m.matches()*/isMatch) {
        ...
        if (parent == null) {
          throw new SystemException("Error: looking for a non-existent parent in tree " + tree + " for \"" + ToString() + "\"");
        }
